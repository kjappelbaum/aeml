...
def make_image(res, objective="1"):
    ...
    for i, point_x in enumerate(outer):
        for j, point_y in enumerate(inner):
            image_m[i][j] = np.sum(
                res[point_x][point_y]["means"][objective].values
                - res[0][0]["means"][objective].values
            )
...
def plot_amp_pz_image(
        ...
        if condition in file:
            ...
            if single_output:
                if "amp" in file:
                    ...
                    amp_image, amp_inner, amp_outer = make_image(load_pickle(file), objective="0")
                if "pz" in file:
                    print(f"pz file {file}")
                    pip_image, pip_inner, pip_outer = make_image(load_pickle(file), objective="0")
            else:
                if "amp" in file:
                    amp_image, amp_inner, amp_outer = make_image(load_pickle(file), objective="0")
                    pip_image, pip_inner, pip_outer = make_image(load_pickle(file), objective="1")
    ...
    fig, ax = plt.subplots(1, 2, sharex="all", sharey="all", figsize=(6, 4))
    ...
    if outdir is not None:
        fig.savefig(
            os.path.join(outdir, f"{raw_conditions}_{str(one_color_scale)}_{str(blur)}.pdf"),
            bbox_inches="tight",
        )
        plt.close(fig)
    else:
...
def compute_double_output_maps(indir, outdir):
    if not os.path.exists(outdir):
        os.makedirs(outdir)

    all_files = glob(os.path.join(indir, "*_full_2"))

    all_conditions = get_all_conditions(all_files)

    for condition in all_conditions:
        try:
            plot_amp_pz_image(condition, all_files, outdir=outdir, single_output=False)
...
@click.argument("indir", type=click.Path(exists=True))
@click.argument("outdir", type=click.Path(exists=False))
def compute_single_output_maps(indir, outdir):
    if not os.path.exists(outdir):
        os.makedirs(outdir)

    all_files = glob(os.path.join(indir, "*_full_long_no_dt"))

    all_conditions = get_all_conditions(all_files)

    for condition in all_conditions:
        try:
            plot_amp_pz_image(condition, all_files, outdir=outdir, single_output=True)
        except Exception as e:
            print(traceback.format_exc())
            print(e)
            pass
